...
function kNN(distanceM, trainLabels, testLabels, testNames, k)
	...
	local accuracy_num = torch.Tensor(4):zero()
	local accuracy = torch.Tensor(4):zero()
	local type_num = torch.Tensor(4):zero()
	local confusionM = torch.Tensor(4,4):zero()
	...
	for i = 1, testSize do
		...
		if k == 3 then
			local knn_3_type_num = torch.Tensor(4):zero()
			...
			confusionM[{ {testLabels[i]}, ni[4] }] = confusionM[{ {testLabels[i]}, ni[4] }] + 1
			if ni[4] == testLabels[i] then
